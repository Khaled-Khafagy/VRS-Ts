...
const server = new Server(
    { name: 'vrs-jira-zephyr', version: '1.0.0' },
    { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === 'push_test_cases_to_zephyr') {
            ...
            const testCases = args?.test_cases as TestCase[];
        ...
        if (name === 'create_test_cycle') {
            const issueKey = args?.issue_key as string;
            const projectKey = args?.project_key as string;
            const cycleName = args?.cycle_name as string;
            const description = args?.description as string;
            const testCaseKeys = args?.test_case_keys as string[];

            const cycle = await zephyr.createTestCycle(projectKey, cycleName, description, testCaseKeys, issueKey);

            await jira.addRemoteLink(
                issueKey,
                `Test Cycle: ${cycleName}`,
                cycle.url,
                'Test Cycle'
            );
