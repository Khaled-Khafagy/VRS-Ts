...
CRITICAL: Start your response IMMEDIATELY with the findings. NO introductory text.`;
    ...
    async generateTestCases(ticketKey, summary, description, acceptanceCriteria, reviewFindings) {
        const systemPrompt = 'You are a professional QA engineer following industry standards. ' +
...
Each item must have:
{
  ...
  "expected_result": "[Specific, measurable outcome]"
    ...
    async refineReview(ticketKey, summary, description, acceptanceCriteria, currentFindings, userMessage) {
        const systemPrompt = "You are a QA analyst. When updating findings, respond ONLY with the formatted findings starting with '**📋'. " +
...
- Start IMMEDIATELY with "**" when updating findings — NO preamble.`;
    ...
    async refineTestCases(ticketKey, summary, description, acceptanceCriteria, currentTestCases, userMessage) {
        const systemPrompt = 'You are a QA engineer. When updating test cases, respond ONLY with a valid JSON array starting with [. ' +
